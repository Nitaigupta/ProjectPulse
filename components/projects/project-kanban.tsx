...
setColumns(initialColumns);
    ...
    const onDragEnd=useCallback(
        ...
        async(result:DropResult)=>{
            ...
            const sourceColumn=newColumn.find(col=>col.id===source.droppableId)
            const destColumn=newColumn.find(col=>col.id===destination.droppableId)
            ...
            if(destinationTasks.length===0){
                ...
                newPosition=(destinationTasks[destination.index-1].position + 
                    destinationTasks[destination.index].position) / 2;
            }
            ...
            destColumn.tasks.splice(destination.index,0,updatedTasks);
    ...
    return <div className="flex gap-4 h-full md:px-4 overflow-x-auto">
        <DragDropContext onDragEnd={onDragEnd} >
          ...
          { columns.map((column)=>(
            <div key={column.id} className="flex flex-col min-w-60 w-80 bg-gray-50 dark:bg-gray-900">
                    ...
                    <Droppable droppableId={column.id}>
                        {
                            (provided)=>(<div {...provided.droppableProps} ref={provided.innerRef} className="flex-1 rounded-lg p-2">
                                {
