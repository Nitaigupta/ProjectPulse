...
export const EditTaskDialog = ({ project, task }: Props) => {
  ...
  const handleOnSubmit = async (data: TaskFormValues) => {
    try {
      setPending(true);
      await updateTask(task.id, data, workspaceId! as string, project.id);
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Enter task name" {...field}></Input>
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
